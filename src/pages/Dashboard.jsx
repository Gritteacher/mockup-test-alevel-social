...
export default function Dashboard() {
  ...
  return <div className="page dashboard-page">
    ...
    <div className="content-grid dashboard-content"><section className="panel recent-panel"><div className="section-title"><div><h2>ผลสอบล่าสุด</h2><p>ประวัติการทำข้อสอบของคุณ</p></div></div>{attempts.length === 0 ? <div className="empty-state">ยังไม่มีประวัติการสอบ</div> : attempts.slice(0, 3).map(a => { const score = scoreOf(a); return <div className="activity" key={a.id}><span className="activity-icon"><BookOpen /></span><div><b>{a.exam_sets?.title || 'ชุดข้อสอบ'}</b><span><Clock3 /> {new Date(a.created_at).toLocaleDateString('th-TH')}</span></div><strong className={score >= 70 ? 'good' : 'medium-score'}>{score}/100</strong></div> })}</section><section className="panel practice-mini"><div className="section-title"><div><h2>ฝึกเพื่อรับโควตา</h2><p>สะสมให้ครบ 100 คะแนน</p></div><Dumbbell /></div><div className="points-circle"><b>{wallet.practice_points % 100}</b><span>/ 100</span></div><Link to="/practice" className="button secondary">ไปฝึกทำโจทย์</Link></section></div>
  </div>
}
