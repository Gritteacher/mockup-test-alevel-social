...
export default function Guide() {
  return <div className="guide-page"><header className="guide-header"><Link to="/" aria-label="กลับหน้าหลัก"><ArrowLeft/></Link><div><span className="brand-mark">M</span><b>คู่มือการใช้งาน</b></div></header><main><div className="guide-intro"><span>เริ่มต้นใช้งานได้ง่าย ๆ</span><h1>วิธีใช้งาน Mock Exam</h1><p>ทำตามขั้นตอนด้านล่างเพื่อเริ่มจำลองสอบ A-Level สังคม</p></div><div className="guide-steps">{steps.map(([Icon,title,text],i)=><article key={title}><span className="guide-number">{i+1}</span><span className="guide-icon"><Icon/></span><div><h2>{title}</h2><p>{text}</p></div></article>)}</div><Link className="button primary guide-start" to="/login">เริ่มใช้งาน <ArrowRight/></Link></main></div>
}
